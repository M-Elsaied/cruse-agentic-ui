...
async def list_systems(
    ...
    try:
        ...
        try:
            ...
            result["custom_networks"] = {
                "my_networks": [
                    {"name": n.name, "slug": n.slug, "network_path": network_key(n.created_by, n.slug)} for n in owned
                ],
                "shared_networks": [
                    {"name": n.name, "slug": n.slug, "network_path": network_key(n.created_by, n.slug)} for n in shared
                ],
            }
        except Exception:  # pylint: disable=broad-exception-caught
            logger.debug("Could not load custom networks for /api/systems")
            result["custom_networks"] = {"my_networks": [], "shared_networks": []}
...
@app.get("/api/networks", response_model=NetworkListResponse)
...
@app.post("/api/networks/validate")
...
async def startup():
    ...
    # Materialize custom networks from DB to disk before factory init
    try:
        from apps.cruse.backend import network_materializer  # pylint: disable=import-outside-toplevel

        network_materializer.ensure_generated_dir()
        factory = get_session_factory()
        if factory is not None:
            async with factory() as db:
                nets = await AgentNetworkRepository(db).list_all_active()
                network_materializer.materialize_all(nets)
