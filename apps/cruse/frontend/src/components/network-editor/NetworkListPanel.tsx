    ...
    try {
      const res = await authFetch(`${API_BASE}/api/networks`, {
        ...
        body: JSON.stringify({
          ...
          slug: newSlug,
          hocon_content: [
            '# LLM config is inherited from the platform (registries/llm_config.hocon).',
            '# Uncomment below to override for this network:',
            '# llm_config {',
            '#   class = "openai"',
            ...
            '{',
            '  metadata {',
            '    description = "My custom agent network."',
            '    tags = ["custom"]',
            '    sample_queries = [',
            ...
            '',
            '  tools = [',
            '    # Front-man agent — the entry point of the network.',
            '    # It has no "parameters" in its function block.',
            '    {',
            '      name = "assistant"',
            '      function {',
            ...
            'When appropriate, delegate to your tools.',
            '""" ${aaosa_instructions}',
            '      tools = ["helper"]',
            '    }',
            '',
            '    # A sub-agent called by the front-man.',
            '    {',
            '      name = "helper"',
            '      function = ${aaosa_call}',
            '      instructions = """',
            'You are a knowledgeable helper.',
            'Provide concise, accurate information about the given topic.',
            '""" ${aaosa_instructions}',
